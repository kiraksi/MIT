...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for char in upper:
            if (ord(char)+shift - 65) < 26:
                shift_dict[char] = chr(ord(char) + shift)
            else:
                shift_dict[char] = chr(ord(char) + shift - 26)

        lower = string.ascii_lowercase
        for char in lower:
            if (ord(char)+shift - 97) < 26:
                shift_dict[char] = chr(ord(char) + shift)
            else:
                shift_dict[char] = chr(ord(char) + shift - 26)
...
def decrypt_story():
    ...
    decrypt_message to return the appropriate shift value and unencrypted story 
    string.
    '''
