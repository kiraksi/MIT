...
def mix_precedence(sym, expected, include_pow=False):
    import random
    lab.type = None
    lab.isinstance = None
    ...
    cprec = {c: c.precedence for c in cn}
    cwrap = {c: c.right_parens for c in cn}
    cwrapL = {}

    if include_pow:
        cwrapL = {c: c.left_parens for c in cn}
    ...
    for wrap_val in [True, False]:
        for c in cn:
            c.precedence = rnum
            c.right_parens = wrap_val
            if include_pow:
                c.left_parens = wrap_val
    ...
    for c in cn:
        ...
        c.right_parens = cwrap[c]
        if include_pow:
            c.left_parens = cwrapL[c]
    lab.type = type 
    lab.isinstance = isinstance

    no_paren = expected_str.replace("(", "").replace(")", "")
    if no_paren not in bad_strs:
        assert False, "Your code got the expected result for __str__ but is type checking somewhere."
...
def test_style_binop():
    for bin_class in [lab.BinOp, lab.Add, lab.Sub, lab.Mul, lab.Div]:
        ...
        for left, right in (('x', 'y'), (.5, -.5), (-1, 'z'), ('z', 1.9)):
            obj = bin_class(left, right)
            name = obj.__class__.__name__
            assert isinstance(obj.left, lab.Symbol), f"{name}({left=}, {right=}): left attribute is not a Symbol!"
            assert isinstance(obj.right, lab.Symbol), f"{name}({left=}, {right=}): right attribute is not a Symbol!"
            with pytest.raises(Exception) as e:
                bin_class.left
            assert e.type == AttributeError, f"{name} has unexpected class attribute left {bin_class.left}"
            with pytest.raises(Exception) as e:
                bin_class.right
            assert e.type == AttributeError, f"{name} has unexpected class attribute right {bin_class.right}"
...
def test_style_attributes():
    ...
    for x in objs:
        ...
        for att in dir(x):
            assert getattr(x, att) != name, f"Unexpected name attribute {att} stored in class {name}"

    for c, o in [(lab.Add, "+"), (lab.Sub, "-"), (lab.Mul, "*"), (lab.Div, "/")]:
        x = c("a", "b")
        n = x.__class__.__name__
        assert any(getattr(c, att)== o for att in dir(c)), f"Expected {n} to store operand {o} as class attribute!"
    
    for c in [lab.BinOp, lab.Var, lab.Num, lab.Symbol, lab.Add, lab.Sub, lab.Mul, lab.Div]:
        assert isinstance(c.precedence, (int, float))
        assert isinstance(c.right_parens, bool)
...
def test_style_repetition():
    ...
    for i, c1 in enumerate(cn):
        for c2 in cn[i+1:]:
            for f in ['__add__', '__radd__', '__mul__', '__rmul__', '__sub__', '__rsub__', '__truediv__', '__rtruediv__']:
                assert getattr(c1, f) == getattr(c2, f), "Be careful to avoid unnecessary repetition!"
...
def test_eval_00():
    ...
    result = inp.eval({'z': 693, 'y': -71, 'x': -391})
    ...
    result = inp.eval({'z': 816, 'y': 732, 'x': -225})
    expected = -134395200
    assert abs(result/expected - 1) <= 1e-4
    with pytest.raises(NameError):
        inp.eval({'z': 693, 'y': -71, })
    ...
    result = inp.eval({'z': 984, 'A': -801, 'x': -880, 'y': 96})
    expected = -1815480
    assert abs(result/expected - 1) <= 1e-4
    with pytest.raises(NameError):
        inp.eval({'z': 693, 'y': -71, })
...
def test_eval_01():
    ...
    assert lab.Mul.eval == lab.Div.eval == lab.Add.eval == lab.Sub.eval == lab.BinOp.eval, f'Correct but avoid unnecessary repetition!'



def test_eq_00():
    expressions = [(lab.Var('x'), lab.Var('x'), True), 
                   ...
                   (lab.Add(lab.Num(10), lab.Add(lab.Num(4), lab.Var('x'))), lab.Add(lab.Num(10), lab.Add(lab.Num(4), lab.Var('x'))), True),
                   ...
                   (lab.Sub(lab.Num(10), lab.Add(lab.Num(4.0), lab.Var('x'))), lab.Sub(lab.Num(10), lab.Add(lab.Num(4), lab.Var('x'))), True),
                   ...
                   (lab.Div(lab.Num(10.9), lab.Add(lab.Num(4), lab.Var('x'))), lab.Div(lab.Num(10.9), lab.Add(lab.Num(4), lab.Var('x'))), True),
                   (lab.Mul(lab.Num(-4), lab.Var('x')), lab.Mul(lab.Num(-4.0), lab.Var('x')), True),
                   (lab.Mul(lab.Var('y'), lab.Num(5)), lab.Mul(lab.Num(5), lab.Var('y')), False),
                   (lab.Mul(lab.Num(10), lab.Add(lab.Num(4), lab.Var('x'))), lab.Mul(lab.Num(10), lab.Add(lab.Num(4), lab.Var('x'))), True),
    ...
    for e1, e2, exp in expressions:
        assert (e1 == e2) is exp, f'{repr(e1)} == {repr(e2)} should be {exp} but got {not exp}!'

    for i, express1 in enumerate(expressions):
        for express2 in expressions[i+1:]:
            assert express1[0] != express2[0], f'{repr(express1[0])} != {repr(express2[0])} but got True!'
            assert express1[1] != express2[1], f'{repr(express1[1])} != {repr(express2[1])} but got True!'

    assert lab.Var.__eq__ != lab.Num.__eq__, "Correct functionality but type checking"
    for c, o in [(lab.Add, "+"), (lab.Sub, "-"), (lab.Mul, "*"), (lab.Div, "/")]:
...
def test_deriv_00():
    ...
    expected = lab.Div(lab.Sub(lab.Mul(lab.Num(5), lab.Num(1)), lab.Mul(lab.Var('x'), lab.Num(0))), lab.Mul(lab.Num(5), lab.Num(5)))
    assert symbol_rep(result) == symbol_rep(expected), f'{result} != {expected}'

    result = lab.Mul(lab.Var('x'), lab.Num(5)).deriv('x')
    expected = lab.Add(lab.Mul(lab.Num(5), lab.Num(1)), lab.Mul(lab.Var('x'), lab.Num(0)))
    assert symbol_rep(result) == symbol_rep(expected), f'{result} != {expected}'
    ...
    assert lab.Mul.deriv != lab.Div.deriv, f"Correct functionality but avoid unnecessary type checking / repetition!"

    exp = lab.Mul(lab.Mul(lab.Var('x'), lab.Var('x')), lab.Var('y'))
    cn = [lab.Add, lab.Sub, lab.Mul, lab.Div]
    for i, c1 in enumerate(cn):
        for c2 in cn[i+1:]:
            e1 = c1.deriv(exp, 'x')
            e2 = c2.deriv(exp, 'x')
            if {c1, c2} != {lab.Add, lab.Sub}:
                assert e1 != e2, f"Correct functionality but code for {c1} found in unexpected class {c2}. Avoid type checking / unnecessary repetition!"
...
def test_simplify_00():
    ...
    result =  lab.Div(lab.Num(2), lab.Num(2)).simplify()
    expected = lab.Num(1.0) 
    assert symbol_rep(result) == symbol_rep(expected)

    result =  lab.Div(lab.Var('x'), lab.Var('x')).simplify()
    expected = lab.Div(lab.Var('x'), lab.Var('x'))
    assert symbol_rep(result) == symbol_rep(expected), f'Make sure to not oversimplify!'

    result =  lab.Div(lab.Var('x'), lab.Num(0)).simplify()
    ...
    result =  lab.Sub(lab.Num(0), lab.Var('x')).simplify()
...
def test_simplify_02():
    ...
    assert symbol_rep(result) == symbol_rep(expected)
    assert lab.Num.simplify == lab.Var.simplify, f"Correct functionality but avoid unnecessary repetition!"

    exp = lab.Div(lab.Num(1), lab.Sub(lab.Num(0), lab.Sub(lab.Var('x'), lab.Add(lab.Sub(lab.Num(0), lab.Mul(lab.Div(lab.Num(0), lab.Var('x')), lab.Num(1))), lab.Num(0)))))
    cn = [lab.Add, lab.Sub, lab.Mul, lab.Div]
    for i, c1 in enumerate(cn):
        for c2 in cn[i+1:]:
            e1 = c1.simplify(exp)
...
def test_parse_00():
    ...
    expected = lab.Add(lab.Var('x'), lab.Div(lab.Num(-.5), lab.Var('x')))
    assert symbol_rep(result) == symbol_rep(expected)
...
def test_pow():
    ...
    exp = lab.Pow(lab.Add('x', 'y'), 'z')
    expected = ("Pow(Add(Var('x'), Var('y')), Var('z'))", '(x + y) ** z')
    assert symbol_rep(safe_eval(repr(exp))) == symbol_rep(safe_eval(expected[0]))
    assert str(exp) == expected[1]
    mix_precedence(exp, expected, True)

    exp = lab.Var('x') ** lab.Var('y') ** lab.Var('z')
    expected = ("Pow(Var('x'), Pow(Var('y'), Var('z')))", "x ** y ** z")
    assert symbol_rep(safe_eval(repr(exp))) == symbol_rep(safe_eval(expected[0]))
    assert str(exp) == expected[1]
    mix_precedence(exp, expected, True)

    exp = lab.Pow('x', lab.Add('y', 'z'))
    expected = ("Pow(Var('x'), Add(Var('y'), Var('z')))", 'x ** (y + z)')
    assert symbol_rep(safe_eval(repr(exp))) == symbol_rep(safe_eval(expected[0]))
    assert str(exp) == expected[1]
    mix_precedence(exp, expected, True)

    exp = lab.Pow(lab.Pow(lab.Num(2), lab.Num(3)), lab.Num(4))
    expected = ("Pow(Pow(Num(2), Num(3)), Num(4))", '(2 ** 3) ** 4')
    assert symbol_rep(safe_eval(repr(exp))) == symbol_rep(safe_eval(expected[0]))
    assert str(exp) == expected[1]
    mix_precedence(exp, expected, True)
    ...
    inp = lab.Pow(lab.Var('x'), lab.Num(2))
    result = inp.eval({'x': 237})
    expected = 56169
    assert abs(result/expected - 1) <= 1e-4
    with pytest.raises(NameError):
        inp.eval({})
...
def test_pow_attributes():
    assert issubclass(lab.Pow, lab.BinOp), f"Pow is not a subclass of BinOp!"
    ...
    x = lab.Pow("a", "b")
    assert x == lab.Pow(lab.Var("a"), lab.Var("b"))
    assert x != lab.Pow(lab.Var("b"), lab.Var("a"))
    assert lab.Pow(lab.Var("a"), lab.Num(0)) == lab.Pow(lab.Var("a"), lab.Num(0.0))
    assert lab.Pow(lab.Var("a"), lab.Num(0)) != lab.Num(1)
    assert lab.Pow(lab.Var("a"), lab.Num(0)) != lab.Sub(lab.Var("a"), lab.Num(0))
    assert lab.Pow.__eq__ == lab.BinOp.__eq__
    assert any(getattr(x, att)== '**' for att in dir(x)), f"Expected Pow to store operand ** as class attribute!"
    assert len(dir(x)) == len(dir(lab.Pow)) + 2

    for c in [lab.Pow, lab.BinOp, lab.Var, lab.Num, lab.Symbol, lab.Add, lab.Sub, lab.Mul, lab.Div]:
        assert isinstance(c.precedence, (int, float))
        assert isinstance(c.right_parens, bool)
        assert isinstance(c.left_parens, bool)
